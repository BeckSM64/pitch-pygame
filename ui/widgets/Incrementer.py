...
class Incrementer:
    def __init__(self, options):
        ...
        self.leftArrow = Arrow(
            self.LEFT_ARROW_X, self.LEFT_ARROW_Y, self.LEFT_ARROW_ROTATION
        )
        self.rightArrow = Arrow(
            self.RIGHT_ARROW_X, self.RIGHT_ARROW_Y, self.RIGHT_ARROW_ROTATION
        )
    ...
    def draw(self, screen):
        ...
        for option, width in activeTextMap.items():
            self.TEXT_X_POSITION = (
                (self.LEFT_ARROW_X + self.incrementerWidth)
                - (self.incrementerWidth / 2)
                - (width / 2)
            )
            self.TEXT_Y_POSITION = (
                ((Arrow.height / 2)) - (self.maxTextHeight / 2)
            ) + self.LEFT_ARROW_Y
